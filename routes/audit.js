...
const KANBAN_TTL     = 30 * 60 * 1000;

function normFase(s) {
  return (s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().replace(/\s+/g, ' ').trim();
}
function matchFase(stage, list) {
  ...
  try {
    const data = await cache.getOrFetch(key, async () => {
      ...
      transactions.filter(t => t.entry_type === 'income').forEach(t => {
        ...
        if (!lid) return;
        const ex  = lastTx[lid];
        const dt  = t.date_payment || t.date_due || '';
      ...
      for (const l of lawsuits) {
        ...
        if (isCobranca) {
          ...
          if (!txMes.length) {
            ...
            ok.push({ ...entry, lancamentosDoMes: txMes.length, totalDoMes: txMes.reduce((s, t) => s + Number(t.amount || 0), 0) });
          }
        } else {
      ...
      const result = {
        mes, criticos, monitoramento, ok,
        resumo: {
          totalProcessosAuditados: criticos.length + monitoramento.length + ok.length,
          criticosCount: criticos.length, monitoramentoCount: monitoramento.length, okCount: ok.length,
...
const AUDR_ZONES = {
  ...
  CAU: ['SALARIO MATERNIDADE PARCELADO','JUDICIAL PARCELADO','ADM PARCELADO','RPV DO MES','RPV DO PROXIMO MES','JUDICIAL IMPLANTADO A RECEBER','ADM IMPLANTADO A RECEBER','SALARIO MATERNIDADE CONCEDIDO','ARQUIVADO IMPROCEDENTE','ARQUIVADO PROCEDENTE','ARQUIVADO POR DETERMINACAO JUDICIAL','IGNORAR ESSA ETAPA','CANCELADO REQUERIMENTO','BENEFICIO CONCEDIDO AGUARDAR','ARQUIVADO ENCERRADO'],
  TAMMYRES: ['FALTA LAUDO FAZER PREVDOC','FALTA LAUDO','PREVDOC'],
  EDUARDO: ['TRABALHISTA'],
};

const AUDR_ZONE_LABEL = {
  MARILIA: 'Ana Marília', LETICIA_OU_ALICE: 'Letícia ou Alice',
  CAU: 'Claudiana (Cau)', TAMMYRES: 'Tammyres', EDUARDO: 'Eduardo',
};
